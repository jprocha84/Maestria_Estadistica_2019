...
summary(datos1)
colMeans(datos1[,-1])
cbind(min(ha), max(ha), mean(ha), sd(ha))
cbind(min(produccion), max(produccion), mean(produccion), sd(produccion))
...
plot(produccion~ha,main="Hectareas cosechadas vs Produccion de caña de azucar", cex.main=0.8, xlab = "Hectareas",cex.lab=0.8,cex.axis=0.8, xlim=c(0,20000),ylim=c(50000,1000000),col="red",cex=0.75)    
abline(modelo1)

# ANOVA
anova(modelo1)

# tests de hipótesis para coefs de regresión
summary(modelo1)
...
qqnorm(standresid, ylab="Residuos estandarizados", xlab="Normal Scores",ylim=c(-2,2),col="red",cex=0.95, pch=19) 
qqline(standresid) 
...
plot(standresid~predichos,ylab="Residuos estandarizados",xlab="Predichos",ylim=c(-2,2),col="red",cex=0.95, pch=19)
abline(0,0) 
abline(-2,0)
abline(2,0)
...
plot(standresid~ha, ylab="Residuos estandarizados", xlab="Hectarea",xlim=c(0,20000),ylim=c(-2,2),col="red",cex=0.95,pch=19) 
abline(0,0)
